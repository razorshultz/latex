...
%borders on them
\usepackage{fancyhdr} %control of headers is better with this package

\usepackage[style=apa]{biblatex} %use biblatex for citations
\addbibresource{cites.bib} %add our file which contains our citations in bibtex format
...
\clearpage{} %\clearpage actually always starts a new "page" while \newpagereally only ends the current column 

\thispagestyle{empty} %suppress page numbering on contents page
\renewcommand{\contentsname}{\centering Contents} %renew contentsname command so we can center the header
\tableofcontents
\newpage
...
%Entries of starred versions of sectional units (e.g \section*{}) can be inserted in the table of contents using \addcontentsline{toc}{<type>}{<content>}, where <type> is section, or subsection
\addcontentsline{toc}{section}{\protect \numberline{} Bands I Like}
